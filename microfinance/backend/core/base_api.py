...
class BaseRequestsAPI:
    ...
    async def arequest_post(self, url, data={}):
        async with httpx.AsyncClient(timeout=self.timeout, verify=ctx) as client:
            return await client.post(url, json=data)

    def request_post(self, url, data={}):
        with httpx.Client(timeout=self.timeout, verify=ctx) as client:
            return client.post(url, json=data)
        
    async def arequest_get(self, url):
        async with httpx.AsyncClient(timeout=self.timeout, verify=ctx) as client:
